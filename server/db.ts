...
export async function upsertUser(user: InsertUser): Promise<void> {
  ...
  try {
    const existingUser = await db.select().from(users).where(eq(users.openId, user.openId)).get();
    ...
    if (existingUser) {
      ...
      else updateSet.lastSignedIn = new Date(); // Default to now

      // Check for role upgrade
      if (user.openId === ENV.ownerOpenId && existingUser.role !== 'admin') {
      ...
      const newUser = { ...user };
      if (newUser.role === undefined && newUser.openId === ENV.ownerOpenId) {
        newUser.role = 'admin';
      }
      if (!newUser.lastSignedIn) newUser.lastSignedIn = new Date();
...
export async function addCartItem(userId: number, productId: number, quantity: number) {
  const db = await getDb();
  if (!db) return null;
  const result = await db.insert(cartItems).values({ userId, productId, quantity }).returning();
  return result;
}
